...
const Header: React.FC<HeaderProps> = ({ isAdmin, view, onNavigate, onLogout }) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#0D1117]/90 backdrop-blur-md border-b border-white/10 py-4 md:py-2.5 px-6 md:px-10 mb-0 shadow-2xl">
      <div className="max-w-5xl mx-auto flex justify-between items-center">
        ...
        >
          <div className="w-12 h-12 md:w-8 md:h-8 mr-3 md:mr-2 relative flex items-center justify-center">
            <div className="absolute inset-0 bg-cyan-500/10 rounded-full opacity-60 group-hover:bg-cyan-500/20 group-hover:scale-110 transition-all duration-300"></div>
            <img 
              src={LOGO_IMAGE_URL} 
              alt="Halezone Logo" 
              className="w-8 h-8 md:w-5 md:h-5 object-contain relative z-10 brightness-110"
            />
          </div>
        ...
        <nav className="flex items-center space-x-1 md:space-x-0">
          <button 
            onClick={() => onNavigate('MAIN')}
            className={`p-3 md:p-2 rounded-full transition-all ${view === 'MAIN' ? 'text-cyan-400 bg-cyan-950/60 border border-cyan-500/30' : 'text-gray-400 hover:bg-white/5 hover:text-cyan-300'}`}
            title="메인"
          >
          ...
          {isAdmin ? (
            <>
              <button 
                ...
                className={`p-3 md:p-2 rounded-full transition-all ${view === 'WRITE' ? 'text-cyan-400 bg-cyan-950/60 border border-cyan-500/30' : 'text-gray-400 hover:bg-white/5 hover:text-cyan-300'}`}
                title="글쓰기"
              >
            ...
            <button 
              ...
              className={`p-3 md:p-2 rounded-full transition-all ${view === 'LOGIN' ? 'text-cyan-400 bg-cyan-950/60 border border-cyan-500/30' : 'text-gray-400 hover:bg-white/5 hover:text-cyan-300'}`}
              title="관리자 로그인"
            >
